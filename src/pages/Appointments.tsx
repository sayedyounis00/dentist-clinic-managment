        ...
        <div className="flex gap-2 flex-wrap">
          {allDates.filter(d => d >= new Date().toISOString().split('T')[0]).slice(0, 7).map(d => (
            <Button key={d} variant={d === selectedDate ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDate(d)}>
              {new Date(d + 'T00:00').toLocaleDateString('ar', { weekday: 'short', month: 'short', day: 'numeric' })}
            </Button>
          ))}
        </div>
      </div>
              ...
              <CardContent className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline">{statusAr(a.status)}</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">{a.type} · {a.duration} دقيقة · {a.notes}</p>
                </div>
                <div className="text-left">
                  <p className="font-medium">{a.time}</p>
                  <p className="text-xs text-muted-foreground">{a.duration} دقيقة</p>
                </div>
                {a.status === 'scheduled' && (
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent>
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="space-y-2"><Label>النوع *</Label><Input value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} placeholder="مثال: تنظيف، استشارة" /></div>
